...
export default function AddData() {
    ...
    const [currentUser, setCurrentUser] = useState<User | null>(null); // State for the currently authenticated user
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
      e.preventDefault();
      if (inputValue.trim() === "" || !currentUser) return; // Prevent empty submissions or if no user is authenticated
  
      try {
        await addDoc(collection(db, "Users"), {
          name: inputValue,
          userId: currentUser.uid, // Store the user ID with the document
        });
    ...
    const fetchUsers = async () => {
      ...
      try {
        // Filter users by the current user's ID
    ...
    useEffect(() => {
      const unsubscribe = onAuthStateChanged(auth, (user) => {
        if (user) {
          setCurrentUser(user);
          fetchUsers(); // Fetch users when authenticated user state changes
        } else {
          setCurrentUser(null);
          setUsers([]); // Clear user list if no user is authenticated
        }
      });
